...
uint16_t color; 
   ...
   if (pole[j][i]!=0) {   
    drawBitmap(ii*28+10, jj*28+3, &fig_cont[uint8_t(abs(pole[j][i])-1)][0], 24, 24,color);     
   }
}
//********************************** 
...
uint16_t color,color_cont;  
   ...
   if (pole[j][i]!=0) {
    drawBitmap(ii*28+10, jj*28+3,&fig[uint8_t(abs(pole[j][i])-1)][0], 24, 24,color);          
    drawBitmap(ii*28+10, jj*28+3,&fig_cont[uint8_t(abs(pole[j][i])-1)][0], 24, 24,color_cont);     
   }
}
//****************************
void show_board() {   
...
//********************************** 
boolean load_usb() {
char s='x'; int i=0,j=0;  boolean load=false; 
...
String str_step(int i) {
  String s="";
   ...
   if (steps[i].type==2) s="0-0";
    ...
    if (abs(steps[i].fig1)>1) s=fig_symb[uint8_t(abs(steps[i].fig1))];   
    ...
    if (steps[i].fig2!=0) {
      s=s+"x";    
      if (abs(steps[i].fig2)>1) s=s+fig_symb[uint8_t(abs(steps[i].fig2))];
    }    
   ...
   for (signed char j=0;j<8;j++) 
    if ((pole[j][i]>0&&nstep%2==1)||(pole[j][i]<0&&nstep%2==0)) { 
     ...
     case fp: //          
      ...
      if (j==6&&pole[j-1][i]==0&&pole[j-2][i]==0) {
        addstep(j,i,j-2,i,0);
      }
      if (getpole(j-1,i-1)&&pole[j-1][i-1]<0) {
       if (j!=1) {
         addstep(j,i,j-1,i-1,0); 
       }
       else {
     ...
     case -fp: //                  
      ...
      if (j==1&&pole[j+1][i]==0&&pole[j+2][i]==0) {
        addstep(j,i,j+2,i,0);
      }
      if (getpole(j+1,i-1)&&pole[j+1][i-1]>0) {
       if (j!=6) {
...
void movestep(int nstep) {     
  ...
  if (steps[nstep].type==1)  //  
   ...
   if (steps[nstep].fig1>0) { //     
    ...
    if (steps[nstep].fig1>0) {
      pole[steps[nstep].x2][steps[nstep].y2]=steps[nstep].type-2;
    }
    else {
      pole[steps[nstep].x2][steps[nstep].y2]=2-steps[nstep].type;
    }
   }
}
//****************************  
